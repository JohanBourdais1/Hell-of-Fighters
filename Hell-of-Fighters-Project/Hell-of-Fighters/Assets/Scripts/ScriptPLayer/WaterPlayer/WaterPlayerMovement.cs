...
namespace ScriptPLayer.WaterPlayer
{
    public class WaterPlayerMovement : Player,IPunObservable
    {
        private bool _flipX;
        private int _cdHeal=30;
        private float _cdCurrentHeal=30;
        private bool _isHealed;
        public bool attackSpeEnable;
        private int _nbAattack;
        public int cdDef=5;
        public float cdCurrentDef=5;
        public bool isDef;
        PhotonView view;
        ...
        void Update()
        {
            ...
            if (_isHealed)
            {
                _cdCurrentHeal -= 1*Time.deltaTime;
                inputManager.isHealing = false;
            }
            if (_cdCurrentHeal<=0)
            {
                _isHealed = false;
                _cdCurrentHeal = _cdHeal;
            }
        ...
        private void FixedUpdate()
        {
            ...
            if (inputManager.isAttacking1 && isGrounded)
            {
                StartCoroutine(DoAttack1());
                inputManager.isAttacking1 = false;
            }
            else if (inputManager.isAttacking2 && isGrounded)
            {
                StartCoroutine(DoAttack2());
            }
            else if (inputManager.isAttacking3 && isGrounded)
            {
            ...
            if (inputManager.isHealing && !_isHealed && isGrounded)
            {
        ...
        public void ApplyDamage3()
        {
            if (solo)
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols = Physics2D.OverlapCircleAll(checkAttack3Rigth.position, 1.5f, LayerMask.GetMask("IAH"));
                    foreach (var col in cols)
                    {
                ...
                {
                    Collider2D[] colls = Physics2D.OverlapCircleAll(checkAttack3Left.position, 1.5f, LayerMask.GetMask("IAH"));
                    foreach (var col in colls)
                    {
                        col.GetComponent<Health>().TakeDamage(damage3);
                    }
        ...
        public void ApplyDamage5()
        {
            if (solo)
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols = Physics2D.OverlapCircleAll(checkAttack5Rigth.position, 1f, LayerMask.GetMask("IAH"));
                    foreach (var col in cols)
                    {
                ...
                {
                    Collider2D[] colls = Physics2D.OverlapCircleAll(checkAttack5Left.position, 1f, LayerMask.GetMask("IAH"));
                    foreach (var col in colls)
                    {
                        col.GetComponent<Health>().TakeDamage(damage5);
                    }
        ...
        {
            animator.SetBool("IsHealing",inputManager.isHealing);
            yield return new WaitForSecondsRealtime(1f);
            inputManager.isHealing = false;
            animator.SetBool("IsHealing",inputManager.isHealing);
        }
