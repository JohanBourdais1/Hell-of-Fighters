...
namespace ScriptPLayer.FirePlayer
{
    public class FirePlayerMovement : Player
    {
        private bool _flipX;
        ...
        public void ApplyDamage1()
        {
            if (solo)
            {
                ...
                {
                    Collider2D[] cols = Physics2D.OverlapCircleAll(checkAttack1Rigth.position, 2.5f, LayerMask.GetMask("IAH"));
                ...
                {
                    Collider2D[] colls = Physics2D.OverlapCircleAll(checkAttack1Left.position, 2.5f, LayerMask.GetMask("IAH"));
            ...
            else
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols = Physics2D.OverlapCircleAll(checkAttack1Rigth.position, 2.5f, LayerMask.GetMask("Player"));
                    foreach (var col in cols)
                    {
                        Health h = col.GetComponent<Health>();
                        if (h.gameObject!=body.gameObject)
                ...
                {
                    Collider2D[] colls = Physics2D.OverlapCircleAll(checkAttack1Left.position, 2.5f, LayerMask.GetMask("Player"));
                    foreach (var col in colls)
                    {
                        Health h = col.GetComponent<Health>();
                        if (h.gameObject!=body.gameObject)
        ...
        public void ApplyDamage2()
        {
            if (solo)
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols1 = Physics2D.OverlapCircleAll(checkAttack2Rigth.position, 3.7f, LayerMask.GetMask("IAH"));
                    foreach (var col in cols1)
                    {
                        Health h = col.GetComponent<Health>();
                        if (h.gameObject!=body.gameObject)
                        {
                            h.TakeDamage(damage2);
                        }
                    }
                }
                else
                {
                    Collider2D[] cols1 = Physics2D.OverlapCircleAll(checkAttack2Left.position, 3.7f, LayerMask.GetMask("IAH"));
                    foreach (var col in cols1)
                    {
                        Health h = col.GetComponent<Health>();
                        if (h.gameObject!=body.gameObject)
            ...
            else
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols1 = Physics2D.OverlapCircleAll(checkAttack2Rigth.position, 3.7f, LayerMask.GetMask("Player"));
                    foreach (var col in cols1)
                    {
                        Health h = col.GetComponent<Health>();
                        if (h.gameObject!=body.gameObject)
                ...
                {
                    Collider2D[] cols1 = Physics2D.OverlapCircleAll(checkAttack2Left.position, 3.7f, LayerMask.GetMask("Player"));
                    foreach (var col in cols1)
                    {
                        Health h = col.GetComponent<Health>();
                        if (h.gameObject!=body.gameObject)
        ...
        public void ApplyDamage3()
        {
            if (solo)
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols = Physics2D.OverlapCircleAll(checkAttack3Rigth.position, 2.3f, LayerMask.GetMask("IAH"));
                    foreach (var col in cols)
                    {
                ...
                {
                    Collider2D[] colls = Physics2D.OverlapCircleAll(checkAttack3Left.position, 2.3f, LayerMask.GetMask("IAH"));
                    foreach (var col in colls)
                    {
                        col.GetComponent<Health>().TakeDamage(damage3);
                    }
            ...
            else
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols = Physics2D.OverlapCircleAll(checkAttack3Rigth.position, 2.3f, LayerMask.GetMask("Player"));
                    foreach (var col in cols)
                    {
                        col.GetComponent<Health>().TakeDamage(damage3);
                    }
                ...
                {
                    Collider2D[] colls = Physics2D.OverlapCircleAll(checkAttack3Left.position, 2.3f, LayerMask.GetMask("Player"));
                    foreach (var col in colls)
                    {
                        col.GetComponent<Health>().TakeDamage(damage3);
                    }
