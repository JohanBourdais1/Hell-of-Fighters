...
namespace ScriptPLayer.GroundPlayer
{
    public class GroundPlayerMovement : Player
    {
        private bool _flipX;
        ...
        public void ApplyDamage1()
        {
            if (solo)
            {
                ...
                {
                    Collider2D[] cols = Physics2D.OverlapCircleAll(checkAttack1Rigth.position, 0.32f, LayerMask.GetMask("IAH"));
                ...
                {
                    Collider2D[] colls = Physics2D.OverlapCircleAll(checkAttack1Left.position, 0.32f, LayerMask.GetMask("IAH"));
            ...
            else
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols = Physics2D.OverlapCircleAll(checkAttack1Rigth.position, 0.32f, LayerMask.GetMask("Player"));
                    foreach (var col in cols)
                    {
                        col.GetComponent<Health>().TakeDamage(damage1);
                    }
                ...
                {
                    Collider2D[] colls = Physics2D.OverlapCircleAll(checkAttack1Left.position, 0.32f, LayerMask.GetMask("Player"));
                    foreach (var col in colls)
                    {
                        col.GetComponent<Health>().TakeDamage(damage1);
                    }
        ...
        public void ApplyDamage2()
        {
            if (solo)
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols1 = Physics2D.OverlapCircleAll(checkAttack2Rigth.position, 0.32f, LayerMask.GetMask("IAH"));
                ...
                {
                    Collider2D[] cols1 = Physics2D.OverlapCircleAll(checkAttack2Left.position, 0.32f, LayerMask.GetMask("IAH"));
                    foreach (var col in cols1)
                    {
                        col.GetComponent<Health>().TakeDamage(damage2);
                    }
            ...
            else
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols1 = Physics2D.OverlapCircleAll(checkAttack2Rigth.position, 0.32f, LayerMask.GetMask("Player"));
                    foreach (var col in cols1)
                    {
                        col.GetComponent<Health>().TakeDamage(damage2);
                    }
                ...
                {
                    Collider2D[] cols1 = Physics2D.OverlapCircleAll(checkAttack2Left.position, 0.32f, LayerMask.GetMask("Player"));
                    foreach (var col in cols1)
                    {
                        col.GetComponent<Health>().TakeDamage(damage2);
                    }
        ...
        public void ApplyDamage3()
        {
            if (solo)
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols = Physics2D.OverlapCircleAll(checkAttack3Rigth.position, 2f, LayerMask.GetMask("IAH"));
                    foreach (var col in cols)
                    {
                ...
                {
                    Collider2D[] colls = Physics2D.OverlapCircleAll(checkAttack3Left.position, 2f, LayerMask.GetMask("IAH"));
                    foreach (var col in colls)
                    {
                        col.GetComponent<Health>().TakeDamage(damage3);
                    }
            ...
            else
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols = Physics2D.OverlapCircleAll(checkAttack3Rigth.position, 2f, LayerMask.GetMask("Player"));
                    foreach (var col in cols)
                    {
                        col.GetComponent<Health>().TakeDamage(damage3);
                    }
                ...
                {
                    Collider2D[] colls = Physics2D.OverlapCircleAll(checkAttack3Left.position, 2f, LayerMask.GetMask("Player"));
                    foreach (var col in colls)
                    {
                        col.GetComponent<Health>().TakeDamage(damage3);
                    }
        ...
        public void ApplyDamage4()
        {
            if (solo)
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols = Physics2D.OverlapCircleAll(checkAttack4Rigth.position, 1.7f, LayerMask.GetMask("IAH"));
                    foreach (var col in cols)
                    {
                        Health h = col.GetComponent<Health>();
                        if (h.gameObject!=body.gameObject)
                        {
                            h.TakeDamage(damage4);
                        }
                    }
                }
                else
                {
                    Collider2D[] colls = Physics2D.OverlapCircleAll(checkAttack4Left.position, 1.7f, LayerMask.GetMask("IAH"));
                    foreach (var col in colls)
                    {
                        Health h = col.GetComponent<Health>();
                        if (h.gameObject!=body.gameObject)
            ...
            else
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols = Physics2D.OverlapCircleAll(checkAttack4Rigth.position, 1.7f, LayerMask.GetMask("Player"));
                    foreach (var col in cols)
                    {
                        Health h = col.GetComponent<Health>();
                        if (h.gameObject!=body.gameObject)
                ...
                {
                    Collider2D[] colls = Physics2D.OverlapCircleAll(checkAttack4Left.position, 1.7f, LayerMask.GetMask("Player"));
                    foreach (var col in colls)
                    {
                        Health h = col.GetComponent<Health>();
                        if (h.gameObject!=body.gameObject)
        ...
        public void ApplyDamage5()
        {
            if (solo)
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols = Physics2D.OverlapCircleAll(checkAttack5Rigth.position, 2.7f, LayerMask.GetMask("IAH"));
                    foreach (var col in cols)
                    {
                        Health h = col.GetComponent<Health>();
                        if (h.gameObject!=body.gameObject)
                        {
                            h.TakeDamage(damage5);
                        }
                    }
                }
                else
                {
                    Collider2D[] colls = Physics2D.OverlapCircleAll(checkAttack5Left.position, 2.8f, LayerMask.GetMask("IAH"));
                    foreach (var col in colls)
                    {
                        Health h = col.GetComponent<Health>();
                        if (h.gameObject!=body.gameObject)
            ...
            else
            {
                if (!spriteRenderer.flipX)
                {
                    Collider2D[] cols = Physics2D.OverlapCircleAll(checkAttack5Rigth.position, 2.7f, LayerMask.GetMask("Player"));
                    foreach (var col in cols)
                    {
                        Health h = col.GetComponent<Health>();
                        if (h.gameObject!=body.gameObject)
                ...
                {
                    Collider2D[] colls = Physics2D.OverlapCircleAll(checkAttack5Left.position, 2.8f, LayerMask.GetMask("Player"));
                    foreach (var col in colls)
                    {
                        Health h = col.GetComponent<Health>();
                        if (h.gameObject!=body.gameObject)
